...
export class IntegracaoApiV1ClienteCatalogoController {
  constructor(private readonly gateway: IntegracaoApiGatewayService) {}

  @Get('catalogo')
  @ApiOperation({ summary: 'Contagem de clientes ativos (read-only)' })
  @ApiOkResponse({ schema: { allOf: [{ $ref: getSchemaPath(ApiEnvelopeDto) }] } })
  async catalogo(@Req() req: Request) {
    const data = await this.gateway.clienteCatalogoCount();
