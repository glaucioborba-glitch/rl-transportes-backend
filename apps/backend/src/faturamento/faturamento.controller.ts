...
export class FaturamentoController {
  ...
  criarBoleto(
    ...
    const ip = (req as { ip?: string }).ip || 'unknown';
    const userAgent = req.get('user-agent') || 'unknown';
    return this.faturamentoService.createBoleto(id, dto, user.id, user, ip, userAgent);
  }
  ...
  atualizarBoleto(
    ...
    const ip = (req as { ip?: string }).ip || 'unknown';
    const userAgent = req.get('user-agent') || 'unknown';
    return this.faturamentoService.updateBoleto(boletoId, dto, user.id, user, ip, userAgent);
  }
}
