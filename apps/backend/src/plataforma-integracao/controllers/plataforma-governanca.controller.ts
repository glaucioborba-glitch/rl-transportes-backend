...
export class PlataformaGovernancaController {
  ...
  async obterConsumo() {
    return {
      success: true,
      data: {
        porCliente: await this.consumoStore.agregarPorCliente(),
        ultimasRequisicoes: await this.consumoStore.ultimos(80),
      },
    };
  }
  ...
  async estatisticas() {
    const regs = await this.consumoStore.ultimos(5000);
    ...
    return {
      success: true,
      data: {
        rotasTop: await this.consumoStore.rotasMaisChamadas(15),
        erros4xx: err4,
        erros5xx: err5,
  ...
  async seguranca() {
    return {
      success: true,
      data: {
        incidentesRecentes: await this.consumoStore.incidentes(40),
