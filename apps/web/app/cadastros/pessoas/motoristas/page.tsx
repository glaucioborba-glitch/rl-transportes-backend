...
import { MotoristaCard } from "./components/motorista-card";
import {
...
import { listCadastrosMotoristas } from "@/lib/api/cadastros-motoristas-client";
    ...
    <div className="space-y-6">
        ...
        <div className="flex gap-2">
          ...
          {canCreate ? (
            <Button
              variant="default"
              size="sm"
              onClick={() => router.push("/cadastros/pessoas/motoristas/novo")}
            >
      ...
      <div className="flex flex-wrap items-center gap-3">
        <div className="relative max-w-md flex-1">
          ...
          <Input
            placeholder="Buscar por nome, CPF, transportadora..."
            value={search}
      ...
      {loading ? <MotoristasSkeleton /> : null}
      {!loading && error ? (
        <WidgetError title="Não foi possível carregar motoristas" onRetry={refetch} />
      ) : null}
      {!loading && !error && motoristas.length === 0 ? <MotoristasEmptyState /> : null}
        ...
        <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
          {motoristas.map((motorista) => (
            <MotoristaCard
              ...
              motorista={motorista}
              canEdit={canEdit}
              onEdit={() => router.push(`/cadastros/pessoas/motoristas/${motorista.id}`)}
              onAuditoria={() =>
                router.push(`/cadastros/pessoas/motoristas/${motorista.id}/auditoria`)
              }
            />
          ))}
        </div>
