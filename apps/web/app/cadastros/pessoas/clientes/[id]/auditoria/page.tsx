...
import { fetchCadastrosClienteAuditoria } from "@/lib/api/cadastros-clientes-client";
...
export default function AuditoriaClientePage({ params }: Props) {
  const router = useRouter();
  const { data: historico, loading, error, refetch } = useWidgetData(
    () => fetchCadastrosClienteAuditoria(params.id),
    [params.id],
  );
    ...
    <div className="space-y-6">
      <div>
        ...
        <div className="mt-2 flex flex-wrap items-center justify-between gap-3">
          <div>
            <h1 className="text-2xl font-bold">Auditoria do Cliente</h1>
            <p className="text-sm text-muted-foreground">Histórico completo de alterações</p>
          </div>
          <Button variant="outline" size="sm" asChild>
            <Link href={`/cadastros/pessoas/clientes/${params.id}`}>Ver cadastro</Link>
          </Button>
        </div>
      </div>
                ...
                <div className="flex-1">
                  <div className="flex items-center justify-between gap-3">
                    <p className="text-sm font-medium">
                      {entry.action === "CREATE" && "Cliente cadastrado"}
                      {entry.action === "UPDATE" && "Dados atualizados"}
                      {entry.action === "DELETE" && "Cliente inativado"}
